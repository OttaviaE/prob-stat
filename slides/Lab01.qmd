...
È necessario installare `R` (disponibile \color{blue}[https://cran.r-project.org/bin/windows/base/](https://cran.r-project.org/bin/windows/base/)\normalcolor)
 
`RStudio` è l'IDE perfetto per `R`  $\rightarrow$ consente un utilizzo di `R` migliore e più semplice  

`RStudio` si può installare dopo aver installato `R` (disponibile  \color{blue}[https://posit.co/download/rstudio-desktop/](https://posit.co/download/rstudio-desktop/)\normalcolor)
...
I file sorgenti dei pacchetti hanno estensione  \color{blue}`tar.gz`\normalcolor. 
