...
Fornisce conoscenze trasversali sui linguaggi di programmazione (aiuta ad entrare nelle loigiche di programmazione in maniera molto soft)
...
**Script**. File di tipo testuale dove salvare il codice usato per generare gli opggetti presenti nell'ambiente. Si possono combinare codice e commenti (righe che non vengono eseguite e interpretate come codice `#`)
...
#| out-width: 70%
#| fig-align: center
#| fig-cap: "creare una nuovo script"
...
```{r}
# crea una matrice con 3 righe e 4 colonne (i valori sono sistemati per riga)
...
`vector`: definisce il numero di ietrazioni
...
52 carte, ne vengono estratte due. Calcoalre la probabilità che entrambe siano fiori, considerando due scenari: 
...
Numero dei casi possibili: Si ottengono con le disposizioni con ripetizione considerando gli $n = 2$ elementi ($\{T, C\}$) consdierando le $k = 10$ possibili sequenze di $\{T, C\}$: $D_{2, 10} = 2^10 = 1024$
